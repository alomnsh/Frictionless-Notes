...
aiBtn.addEventListener('click', async () => {
    ...
    aiOutput.innerText = "Connecting to AI model server...";
    ...
    try {
        const response = await fetch(proxyUrl + encodeURIComponent(targetApiUrl), {
            ...
            body: JSON.stringify({ 
                inputs: rawBuffer,
                parameters: { max_length: 100, min_length: 35, do_sample: false }
            })
        });
        ...
        if (result && result.error && result.error.includes("loading")) {
            const estimatedTime = result.estimated_time || 10;
            aiOutput.innerText = `AI model is warming up on the server cloud. Retrying automatically in ${Math.round(estimatedTime)} seconds...`;
            // Wait for the server to wake up and try again automatically
            setTimeout(() => aiBtn.click(), estimatedTime * 1000);
            return;
        }
        ...
        if (Array.isArray(result) && result[0] && result[0].summary_text) {
            ...
            aiOutput.innerText = "Server response error: " + JSON.stringify(result);
            return;
        }

        // Break up the real AI's rephrased text into clean bullet points
